...
How does Markdoc join these two to create HTML? It's very elegantly simple. There's three core components: (1) a parser; (2) a transformer; (3) a renderer. You pass your content, and a config file with your custom Tag schemas, into the parser. The parser creates an abstract syntax tree, which you then process through a transformer method into objects the renderer can understand, and then the renderer renders the desired output (HTML, JSX, etc.). 

```javascript
const ast = Markdoc.parse(content);
const content = Markdoc.transform(ast, /* config */);  // the config is where you can provide definitions for custom tags
...
You can also [conditionally render section blocks](https://markdoc.io/docs/tags#built-in-tags), use [partials](https://markdoc.io/docs/partials) to DRY out your Markdown content that is repetitive across files, define and use [variables](https://markdoc.io/docs/variables), and [so on](https://markdoc.io/docs/getting-started). But to me, the beauty of the library is in its simplicity: three steps - parse, transform, render. And away we go!
